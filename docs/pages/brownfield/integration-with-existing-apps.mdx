...
React Native offers a flexible development strategy. Whether you're launching a new application or looking to integrate an existing native app, it provides a powerful solution. You can leverage its capabilities to introduce new features or views incrementally, allowing your team to adopt modern, cross-platform development without a complete rewrite.

In this guide, we will walk through the steps to add a React Native view into an existing native app using the "integrated" approach.
...
This command creates a new directory named `exp` that contains your new Expo project. While you can name the project anything, this guide uses `exp` for consistency. The new project includes an example TypeScript application to help you get started.

## Set up your project structure

A standard React Native project places native code in `/android` and `/ios` directories. This structure may not be feasible when integrating with a large, existing native app.
This guide covers two approaches: moving your native project files into the standard directories or setting up a monorepo.
...
Setting up a monorepo will ensure that Android and iOS scripts will be able invoke commands from Node libraries even with a custom folder structure.
To setup a Yarn monorepo, create a `package.json` file in the root of your project and add the following content:
...
Then run `yarn install` to install the dependencies. This will ensure `node_modules` are installed at the root of your project, and that native scripts will be able to interact with React Native code.
Make sure to change `["exp"]` to the name of the Expo projected you create in the previous step.
